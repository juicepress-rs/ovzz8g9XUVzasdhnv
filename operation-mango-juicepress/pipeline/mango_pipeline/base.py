...
class Pipeline:
    ...
    def __init__(
        ...
        py_spy: bool = False,
        timeout: int = 120 * 60,
        rda_timeout: int = 5 * 60,
        bin_prep: bool = False,
        exclude_libs: bool = True,
        show_dups: bool = False,
        juicepress_args: dict[str, Any] = {},
    ):
        self.target = target
        ...
        self.juicepress_args = juicepress_args
    ...
    def env_merge_firmware(self, result_dir: Path):
        escaped_docker_res = str(result_dir.absolute())
        escaped_out_file = str(result_dir.absolute() / "env.json")
        command = [
